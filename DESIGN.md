...
> 这份文档写给两类读者:想接手这个项目的人,和想判断它值不值得用的人。
> 所以它不只写"做了什么",也写"为什么不那样做"(§5)、
> "踩过哪些坑"(§8)、"哪里还是坏的"(§7)。
...
| §1 | 这是什么、赌的是什么 |
| §2 | 三二进制架构与数据流 |
| §3 | 控制面:治理层次、权限、阶梯、两种封禁入口、合规报告 |
| §4 | 数据面:采样侧、执行侧两级匹配、Go↔eBPF 契约 |
| §5 | 13 项关键决策及不选另一个的原因、资源保护五道闸门 |
| §6 | 数据模型与 SQLite 配置 |
| §7 | **还没修的缺口**(按严重度排序) |
| §8 | **已修的八次事故**及共同模式 |
| §9 | 工程实践与验证边界 |
...
| `xdp-agent` | 执行面:轮询指令,写 XDP 黑名单 map | 是 | 5.8 MB |
| `xdp-sampler` | 观测面:镜像口 1/N 采样,上报流量 | 是 | 6.2 MB |

**采样是旁路的**:它看的是流量副本,恒返回 `XDP_PASS`。执行是业务网卡上另一个独立程序。
两者互不影响 —— 采样器崩了不影响封禁,封禁程序崩了不影响业务转发。

代码量:Go 6,323 行(非测试)+ 2,846 行测试 + 397 行 C(eBPF)。
测试与实现之比接近 1:2,主要投在跨语言契约与并发路径上(见 §8 的原因)。
...
不生成真 PDF 而用浏览器打印:纯 Go 的 PDF 库都要处理中文字体嵌入,
要么二进制涨几 MB,要么在缺字体的机器上出方块。浏览器用系统字体,
中文永不出错。CSV 带 UTF-8 BOM —— 不加中文 Excel 会乱码,这是实际交付
最常见的投诉。

### 3.7 IP 前缀库管理

`/prefixdb` 提供三种导入方式,对应三种现实处境:

| 方式 | 适用 | 要点 |
|---|---|---|
| 在线同步 | 能出网 | 内置 iptoasn + APNIC/RIPE 多源 |
| 上传文件 | 隔离网 | 不是退路 —— 很多政企运维网段根本不出网 |
| 本地覆盖规则 | 数据判错 | 手工维护,优先级最高,不被同步覆盖 |

多源是必要的:单一上游宕机或改格式就会让功能失效,且不同源口径不同
(iptoasn 反映 BGP 公告,RIR 反映注册归属)。

下载到临时文件**再原子改名**:直接写目标文件时中途失败会留下半个库,
而重启会把它当成有效数据加载 —— **半个前缀库比没有更糟**,因为封禁
范围会静默变窄。先解析验证再启用,失败时旧库继续服务。

本地覆盖规则做成文本编辑而非结构化表单(这文件是给人手写的),
保存前校验并**报出错误行号** —— 写坏的文件会让下次启动时整库加载失败。

### 3.8 页面与导航

| 路由 | 页面 | 所需能力 |
|---|---|---|
| `/dashboard` | 概览统计 | dashboard_view |
| `/bans` | 单点封禁列表与审批 | ban_request_view |
| `/scoped` | 范围封禁列表与审批 | ban_request_view |
| `/sampling` | 采样率 + Top 流量 + 一键封禁 | dashboard_view |
| `/prefixdb` | IP 库同步/上传/覆盖规则 | system_config |
| `/audit` | 审计日志 | audit_view |
| `/report` | 合规报告导出 | audit_view |
| `/users` | 用户与密码管理 | user_manage |
| `/approve/:token` | 邮件审批(唯一公网路由) | 令牌本身即凭证 |

导航项与真实页面一一对应,不给死链 —— 角色看不到的页面直接不在导航里,
但**真正的墙仍是后端的 `requireCap`**。
...
| eBPF 嵌入方式 | `go:embed` 编译产物 | 用 `od` 生成 Go 字节数组会让 diff 不可读、且多一份事实源 |
| `.o` 是否入库 | 不入库,构建时生成 | 它必须与 `.c` 同步,入库就有了第二份可能过期的事实源 |
| 前缀展开时机 | 审批下发时重新展开 | 用提交时的快照会让封禁滞后于 BGP 现实;代价是要检测并修正漂移 |
| 范围封禁落库粒度 | 只存选择器 + 数量 | 逐条存上万前缀会让单次操作产生上万行,审批列表不可读 |
| 一条指令 vs 上万条 | 一条指令带前缀列表 | dispatch 是审计与回执的单位;拆开后"这次封禁成功没有"无法回答 |
...
诚实清单,按严重程度排序。已修复的问题移到 §8「事故与修复记录」。
...
**这是当前唯一会随时间累积成事故的缺口**,也是下一步该做的第一件事。

设计要点(尚未实现):agent 侧起一个 ticker,用 `Map.Iterate()` 遍历
`src_ban` / `src_ban_global`,`DecodeValue` 判断 `expires_at` 是否已过
当前 ktime,过期即 `Delete`。同时要把释放量回报控制面以修正配额账面
—— 否则内核已腾出空间而 Tracker 仍认为满。遍历要限速:26 万条全量扫
不能阻塞在途的 map 写入。
...
同理,eBPF 程序**从未真正加载进内核**过 —— 只验证了 bytecode 可被
`cilium/ebpf` 解析、map 齐全。verifier 是否放行、实际转发行为是否正确,
需要真机确认。
...
—— 修 P0 时就付了这个代价:`CreateDispatch` 和 `CreateScopedDispatch`
是两份高度相似的代码。
...
这一节记的是**已经修掉但值得记住**的问题。它们有共同的模式:
不报错、编译通过、测试全绿,而功能实际是死的。这类"静默失效"是
本项目最需要防的东西。

### 8.1 agent 与 eBPF 程序不匹配(曾阻塞整个执行面)

做范围封禁时重构了 `xdp_filter.c`(单级 `ban_list` → 两级
`target_hosts` + `src_ban`),但没同步更新 agent 的写入逻辑:
agent 找已不存在的 `ban_list`,启动即 Fatalf;key 编码也还是旧的 8 字节格式。
**控制面完整可用,执行面实际跑不起来。**

单测没抓到,是因为它们只覆盖纯函数(地址解析、字节序、结构体布局),
**没有任何测试触及 map 加载与写入**。map 名是运行时字符串查找,
编译器帮不上忙。

修复:

- eBPF 侧补上 `src_ban_global`(LPM_TRIE)—— 上次重构只考虑定向封禁,
  把最常用的"封这个源不限目标"场景**在数据结构上就丢了**
- 编码逻辑抽到 `internal/banmap`,能在无内核环境单测
- agent 拆出 `executor.go`,依赖抽象 `mapWriter` 接口而非 `*ebpf.Map`,
  用内存 fake 完整测试两条执行路径
- 补上 §4.3 的四条契约测试,其中两条**直接解析 C 源码比对**

### 8.2 TTL 用错时钟基准(所有封禁会变成永久)

XDP 只能拿到 `bpf_ktime_get_ns()`(系统 uptime),而 agent 传的是 Unix 纳秒。
Unix 纳秒(~1.78e18)远大于任何 uptime,所以**每一条封禁都被判成"未过期"
而永久生效** —— 一条 10 分钟的临时封禁会永远留在 map 里。

修复:从 `/proc/uptime` 读**系统**启动时刻做换算。注意不能用进程启动时刻,
那样在已运行数天的机器上所有封禁会立刻失效 —— 同一个坑的另一个方向。

### 8.3 LPM key 未归一化(规则写进去永远匹配不上)

`203.0.113.5/24` 这种主机位非零的前缀,内核插入位置与查询时的
最长匹配算法不一致。规则确实写进 map 了,但永远匹配不到任何包。

修复:强制 `netip.Prefix.Masked()`。

### 8.4 发布二进制内嵌空 bytecode

仓库跟踪 `obj/*.o` 的 0 字节占位文件(为了让无 clang 的机器也能编译
Go 部分),但发布流程没有强制先 `make bpf` —— v0.26 及之前的
agent/sampler 内嵌空 bytecode,启动报 `bytecode 为空`。
**错误发生在客户机器上,而不是构建时。**

修复:

- `.o` 从版本库移除(构建产物,不该有第二份事实源)
- `make bpf-check` 断言 `.o` 非空,`build`/`dist` 都依赖它
- `make bpf` 补 `-I/usr/include/$(ARCH_TRIPLET)`:Debian/Ubuntu 把 `asm/`
  放在架构子目录,不加会 `asm/types.h not found`
- 补 `TestEmbeddedBytecodeHasRequiredMaps` 真正解析内嵌 bytecode

### 8.5 包长算错(上报字节数全是垃圾)

采样侧用了 `ctx->data_meta - data`。`data_meta` 指向元数据区
(XDP 程序间传递自定义数据用),**与包长毫无关系**。

这个错误此前**编译不过所以一直藏着** —— 直到真正编译 eBPF 才暴露。
修复:改用 `data_end - data`。

### 8.6 采样退化成"全采或全不采"

随机种子只用 `ctx->rx_queue_index`,同一队列上每个包算出的随机数
**完全相同**。结果是整队列全采或全不采,采样率 1/N 形同虚设。

修复:混入五元组与 ktime。顺带给 `rate == 0` 加兜底 —— 除零会被
verifier 直接拒绝加载。

### 8.7 会话表并发写导致进程退出

会话表原本是裸 `map[string]uint`。Gin 每请求一个 goroutine,
并发登录触发 `fatal error: concurrent map writes` —— **这个 panic 无法
recover,进程直接退出**。等于一个可远程触发的拒绝服务:多人同时点登录即可。

修复:`RWMutex` + TTL + 后台 reaper。顺带补上两个安全缺口:
登出以前**没真正吊销** token、改密码后旧 cookie 仍然畅通。

### 8.8 SQLite 什么都没配

初版 `Open()` 只给了文件路径。四个必须的设置一个都没有:

| 缺什么 | 后果 |
|---|---|
| `journal_mode=WAL` | 一次审批写阻塞所有仪表板读 |
| `busy_timeout` | 并发写直接 `SQLITE_BUSY` → 随机 500 |
| 连接池上限 | 嵌入库连接堆高只把争抢从 Go 层挪到文件锁层 |
| `PrepareStmt` | pprof 显示 SQL 反复解析占 CPU 17% |

修复后还加了一道:`Open()` **回读 `PRAGMA journal_mode` 验证 WAL 真生效**
—— pragma 名写错时驱动静默忽略,不验证会以为开了其实没开。

### 共同模式

八个问题里有六个是**静默失效**:不报错、不崩溃、看起来在工作。
这决定了本项目的测试策略重点不是"覆盖率",而是**把跨语言、跨进程的
契约变成可执行的断言**(§4.3 那四条),以及**在提交时就拒绝**而不是
等运行时失败(§5 的五道闸门)。
...
```
make bpf        clang → cmd/*/obj/*.o,go:embed 内嵌(需 clang + libbpf-dev)
make bpf-check  断言 .o 非空 —— build/dist 都依赖它
make build      三个静态二进制
make check      go vet + go test -race
make dist       交叉编译 linux/{amd64,arm64} + SHA256SUMS
make release    bpf → check → dist,发布前的完整流程
...
**测试策略**:重点不是覆盖率,而是把跨语言、跨进程的契约变成可执行断言。
`go test -race` 全绿。fuzz 覆盖 `ParseIPv4Prefix`(执行器信任边界)
与 `parseRate`(Web 输入)。
...
| 层面 | 状态 |
|---|---|
| Go 单元/契约测试 | ✅ 全绿,含 race |
| eBPF 编译 | ✅ clang 产出 .o,map 与容量已核对 |
| bytecode 可解析 | ✅ `cilium/ebpf` 解出 spec,map 齐全 |
| 控制面端到端 | ✅ HTTP 走通:提交→审批→dispatch payload 正确分流 |
| eBPF 加载进内核 | ❌ 需 root,verifier 是否放行未验证 |
| 真机转发/丢包行为 | ❌ 需真实网卡 |
| XDP 性能 | ❌ 脚本已备,无数据 |
...
这些都是想清楚"哪里会出事故"之后的选择。§8 那八次事故里有六次是
**静默失效**——不报错、不崩溃、看起来在工作。所以这个项目真正的工程
重心不是功能,而是**让错误在构建时而不是在客户机器上暴露**。

§7 的缺口清单是还没做完的部分,其中 P1 那条(过期表项 reaper)
是唯一会随时间累积成事故的,应该最先做。
