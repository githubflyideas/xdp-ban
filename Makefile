...
# 两个二进制:xdp-ban(控制面 + 执行器,吸收了原 xdp-agent)/ xdp-sampler(独立采样器)
.PHONY: all
all: bpf build
...
bpf:
	...
	@echo ">> eBPF 目标文件:"
	@ls -l cmd/xdpban/obj/xdp_filter.o cmd/xdp-sampler/obj/xdp_sampler.o

## bpf-check: 断言待嵌入的 .o 非空(防止发布出内嵌空 bytecode 的二进制)
.PHONY: bpf-check
bpf-check:
	@for f in cmd/xdpban/obj/xdp_filter.o cmd/xdp-sampler/obj/xdp_sampler.o; do \
	  if [ ! -s "$$f" ]; then \
...
dist: bpf-check
	...
	@for arch in amd64 arm64; do \
	  ...
	  for cmd in xdpban:xdp-ban xdp-sampler:xdp-sampler; do \
	    pkg=$${cmd%%:*}; out=$${cmd##*:}; \
...
.PHONY: clean
clean:
	rm -f xdp-ban xdp-sampler coverage.out coverage.html *.db
	rm -rf $(DIST)
	rm -f cmd/xdpban/obj/*.o cmd/xdp-sampler/obj/*.o
