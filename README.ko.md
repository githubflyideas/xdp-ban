...
세 개의 바이너리, 각각 독립 배포 가능:

| 바이너리 | 역할 | root 필요 |
|---|---|---|
| `xdp-ban` | 컨트롤 플레인: 웹 UI, 승인, SQLite | 아니오 |
| `xdp-agent` | 집행: 지시를 폴링하고 XDP 맵에 기록 | 예 |
...
```bash
make bpf      # clang → cmd/*/obj/*.o, go:embed 로 임베드
make build    # 세 개의 정적 바이너리
